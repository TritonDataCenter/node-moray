    ...
    var retry = backoff.call(_connect, {}, function (err, client) {
        ...
        if (!err) {
            log.debug('moray: connected to %s after %d attempts',
                      opts.host, retry.getNumRetries() + 1);
        }
        callback(err, client);
    });
...
function createConnectionRing(opts, cb) {
    ...
    ring.close = function close() {
        ...
        function onClose(conn) {
            ...
            conn.removeAllListeners('unhandledMessage');
            if (++dead === total)
                ring.emit('close');
        }
    ...
    function connect_cb(connect_err, client) {
        ...
        client.on('unhandledMessage', function (msg) {
            if (!client._deadbeef)
                log.warn('unhandled message (dropped): %j', msg);
        });

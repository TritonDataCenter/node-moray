...
MorayClient.prototype.batchPutObject = function batchPut(requests, opts, cb) {
        ...
        requests.forEach(function (r) {
                assert.string(r.bucket, 'bucket');
                assert.string(r.key, 'key');
                assert.object(r.value, 'value');
                assert.optionalObject(r.options, 'options');
                if (r.options) {
                        assert.optionalObject(r.options.headers,
                                              'options.headers');
                }
        });
        ...
        req = client.rpc('batchPutObject', requests, options);

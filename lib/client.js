...
function MorayClient(options) {
    ...
    this.pool.on('error', function (err) {
        // Only re-emit errors if a consumer is listening.  This prevents
        // single-connection errors in the pool from halting the entire app.
        if (self.listeners('error').length > 0) {
            self.emit('error', err);
        } else {
            self.log.error({err: err}, 'moray error');
        }
    });

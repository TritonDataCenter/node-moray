 ...
 *                       resolvers: ['10.99.99.201'],  // default /etc/resolv.conf
...
function MorayClient(options) {
        ...
        /// This is our "poll and replace pool" method
        var log = this.log;
        function dns_check() {
                ...
                dns.resolve(dns_opts, function (dns_err, ips) {
                        ...
                        // replace with the new one. Suboptimal, but meh,
                        // it doesn't happen that often
                        function _cb(err2, pool2) {
                                if (err2) {
                                        log.error(err2, 'failed to reconnect');
                                } else {
                                        self.pool.close();
                                        self.pool = pool2;
                                        log.info({
                        ...
                        log.debug({
                                domain: dns_opts.domain,
                                ips: ips
                        }, 'DNS records changed, replacing connections');
                        connection.createConnectionPool(pool_opts, _cb);
                });
        }
        var freq = options.dns.checkInterval || 30000;
        var schedDNSCheck = setTimeout.bind(null, dns_check, freq);

        // This is essentially "init"
        connection.createConnectionPool(pool_opts, function (err, pool) {
                if (err) {
                        self.emit('error', err);
                        return;
                }
 ...
 *
 * See https://mo.joyent.com/docs/moray/master/#BatchPutObject for more info.
 *
 * @param {Array} requests - {bucket, key, value} tuples
 * @param {Object} opts - request parameters
 * @param {Function} cb - callback
 */
MorayClient.prototype.batchPutObject = function batchPut(requests, opts, cb) {
        ...
        var client = this.getClient(cb);
        if (client)
                objects.batchPut(client, requests, opts, cb);
};

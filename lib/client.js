...
function MorayClient(options) {
    ...
    // MORAY-247: Without a domain in place to capture any/all errors,
    // listening for them on all opened sockets is needed.
    function subscribeErrors(pool) {
        pool.ring.forEach(function (connRing) {
            connRing.ring.forEach(function (conn) {
                conn.on('error', self.emit.bind(self, 'error'));
            });
        });
    }

    function reconnectCallback(err, pool) {
        if (err) {
            log.error({
                err: err
            }, 'failed to reconnect');
        } else {
            // Here we naively just destroy the old one and replace with the
            // new one. Suboptimal, but it doesn't happen that often.
            self.pool.close();
            self.pool = pool;
            subscribeErrors(pool);
        }
    ...
    function dns_check() {
        ...
        dns.resolve(dns_opts, function (dns_err, ips) {
            ...
            connection.createConnectionPool(pool_opts, reconnectCallback);
        });
    }
